...
async function getAuthToken() {
    try {
        ...
        throw new Error("Failed to authenticate scheduler system.");
    }
}
...
app.get('/schedule', async (req, res) => {
    ...
    try {
        ...
        if (!token) return res.status(401).json({ error: "Unauthorized endpoint access" });
        ...
        res.status(502).json({ error: "Backend computational pipeline crashed", details: error.message });
    }
});
app.listen(PORT, () => {
